 ...
 */
static void afatfs_sdcardReadComplete(operacionBloqueSD_e operation, uint32_t sectorIndex, uint8_t *buffer, uint32_t callbackData)
{
    (void) operation;
    (void) callbackData;
 ...
 */
static void afatfs_sdcardWriteComplete(operacionBloqueSD_e operation, uint32_t sectorIndex, uint8_t *buffer, uint32_t callbackData)
{
    (void) operation;
    (void) callbackData;
...
static void afatfs_cacheFlushSector(int cacheIndex)
{
    ...
    if (cacheDescriptor->consecutiveEraseBlockCount) {
    	iniciarEscrituraBloquesSD(cacheDescriptor->sectorIndex, cacheDescriptor->consecutiveEraseBlockCount);
    }
#endif

    switch (escribirBloqueSD(cacheDescriptor->sectorIndex, afatfs_cacheSectorGetMemory(cacheIndex), afatfs_sdcardWriteComplete, 0)) {
        case SD_OPERACION_EN_PROGRESO:
    ...
    switch (afatfs.cacheDescriptor[cacheSectorIndex].state) {
        ...
        case AFATFS_CACHE_STATE_EMPTY:
            if ((sectorFlags & AFATFS_CACHE_READ) != 0) {
                if (leerBloqueSD(physicalSectorIndex, afatfs_cacheSectorGetMemory(cacheSectorIndex), afatfs_sdcardReadComplete, 0)) {
